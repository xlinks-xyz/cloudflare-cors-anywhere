...
async function handleRequest(request) {
    ...
    if (!isListed(targetUrl, blacklistUrls) && isListed(originHeader, whitelistOrigins)) {
...
async function createModifiedRequest(originalRequest, targetUrl) {
    ...
    ['host', 'origin', 'referer', 'cf-connecting-ip', 'cf-ipcountry', 'x-forwarded-for'].forEach(header => {
...
function handlePreflight(request) {
    const headers = new Headers({
        ...
        "Access-Control-Allow-Methods": request.headers.get("Access-Control-Request-Method") || "GET, POST, PUT, DELETE, OPTIONS",
